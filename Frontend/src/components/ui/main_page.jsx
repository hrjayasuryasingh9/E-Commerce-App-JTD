...
const MainPage = () => {
  ...
  async function getSellerProducts(seller_id) {
    try {
      ...
      const response = await fetch(
        `https://e-commerce-app-jtd-b0ky.onrender.com/api/product/seller/getproducts/${seller_id}`,
        {
          method: "GET",
          headers: {
  ...
  const handleDeleteProduct = async (id) => {
    try {
      const response = await fetch(
        `https://e-commerce-app-jtd-b0ky.onrender.com/api/product/seller/deleteproduct/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
