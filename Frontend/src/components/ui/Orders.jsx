...
const ProductCard = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch(
        `https://e-commerce-app-jtd-b0ky.onrender.com/api/orders/seller/getorders/${sellerId}`,
        {
          method: "GET",
          headers: {
  ...
  const handleDispatch = async (index) => {
    ...
    try {
      const response = await fetch(
        `https://e-commerce-app-jtd-b0ky.onrender.com/api/orders/seller/editstatus/${orderId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
